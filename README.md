...
> 以下数据来自 integration_v2 物流项目（Python，~50 个文件）全量扫描的真实输出。

| 指标 | 数值 | 说明 |
|---|---|---|
| 总发现 | 533 项 | L3 bandit 328 + L6 CLE 205 |
| **P0（崩溃级）** | **0** ✅ | 三层工具交叉确认 |
| **P1（真实高危）** | **32** | 静默吞错（except: pass） |
| P1（设计选择） | 161 | 过宽 except Exception（batch 容错） |
| P2（中危） | 3 | 测试文件 MD5 弱哈希 |
| P3（低危） | 6 | 硬编码绝对路径 |
| **信噪比** | **7.7%** | 41 真实风险 / 533 原始告警 |

**本次验证的关键能力**：
- ✅ L6 CLE 探针：Python 文件扫描（22 个 Python 算子全部生效）
- ✅ L7 冒烟测试：多语言自适应（py_compile 全项目通过）
- ✅ L8 ocr 集成：自动检测 → 不可用自动退回 Inline 模式
- ✅ L4 gitleaks：密钥泄露 0 检出（正常）
- ✅ 分级校准：过宽 except vs 静默吞错 明确区分，不夸大 P0
...
## 2026-09-24 大版本更新（v2.0）

### 本次更新解决的核心问题

**假执行 / 假报告问题**：之前工具报了就信，结果 80% 是误报。本次更新用三层防护解决：

1. **输入校验** — 自动识别文件类型，不会 C 代码存成 .py 被 Python 规则扫
2. **多工具交叉验证** — 同一个问题至少两个工具都报，才算高置信
3. **严重级自动校准** — 按校准准确率自动降级，防止 P0 满天飞
...
| 功能 | 说明 |
|---|---|
| **L8 PEF 第一性原理框架** | 从模糊的"代码审查专家"升级为 P→E→F 三元分解 + 因果链断裂检查 |
| **语言特定审查重点** | C/C++ 查内存/溢出/指针，Python 查注入/硬编码/静默吞错（参考阿里 ocr） |
| **L8 后处理层** | 去重 + 误报过滤 + 严重级校准（P0 必须是崩溃级，否则降级） |
| **本地白盒规则库** | C 12 条 + Python 10 条核心规则，Semgrep/bandit 挂了的 fallback |
| **探针校准测试集** | 40 个用例（Python 20 + C 20），量化探针可信度 |
| **多工具交叉验证** | 同文件+同行号+同类别，只 1 个工具报的自动降级 |
| **严重级自动校准** | 按校准准确率自动调整，准确率 <80% 全部降一级 |
...
| 指标 | 更新前（v1.0） | 更新后（v2.0） |
|---|---|---|
| 总发现数 | 73 项 | 8 项 |
| 误报率 | ~80% | ~0%（校准集实测） |
| 最高严重级 | P0 × 8（全是误报） | P1 × 1（交叉验证后） |
| 最终裁决 | FAIL | WARN |
| 可解释性 | 黑盒：报了就信 | 白盒：触发了哪条规则、为什么 |
| 依赖外部工具 | 100% 依赖 Semgrep/bandit | 有本地 fallback，不依赖在线 |

#### 校准集实测结果（local_rules.py）

| 指标 | 数值 |
|---|---|
| 召回率 | 30%（10 个真 bug 抓到 3 个） |
| 准确率 | 100%（报了 3 个，全是真的） |
| 误报数 | 0/10（假 bug 一个都没报） |

> **fallback 工具的正确姿态**：宁可漏报，不误报。报了就是真的。
...
```
更新前：
  L0-L5（黑盒工具）→ L6 → L7 → L8（模糊审查）→ L9
  问题：黑盒结果直接信，误报满天飞

更新后：
  L0-L5（黑盒工具）
    ↓
  L1/L3 补充：本地白盒规则（fallback）
    ↓
  合并所有 findings
    ↓
  多工具交叉验证（只 1 个工具报的降级）
    ↓
  L8 PEF 框架审查（P→E→F 因果链）
    ↓
...
| | 更新前 | 更新后 |
|---|---|---|
| 依赖 Semgrep | 全靠它，挂了就没了 | ✅ 有 local_rules fallback |
| 依赖 bandit | 全靠它，挂了就没了 | ✅ 有 local_rules fallback |
| 依赖在线规则 | 是 | ✅ 本地有白盒规则库 |
| 依赖 LLM 质量 | 是，看模型心情 | ✅ PEF 框架约束推理 |
| 可信度验证 | 没有 | ✅ 校准集量化准确率 |

---

*v2.0 · 从黑盒到白盒，从假报告到真检测。*
